...
while True:
    ...
    time.sleep(5)
Base64 图片输入支持#
对于图像编辑等模型，输入的图片建议使用线上托管的图像URL地址。如果需要使用本地文件，则需要对本地图片进行 Base64编码，并将编码后的数据作为image_url传递给模型。一个典型的调用方式是：

        # input as base64
        "image_url": [
           image_to_data_url("path/to/local/image.jpg")
        ]
其中image_to_data_url函数可以参考如下实现：
...
image_url	待编辑图片的url地址（或者编码过的base64数据），该参数只适用于支持图片编辑的模型	否	string	https://resources.modelscope.cn/aigc/image_edit.png	确保公网可访问（或者编码过的base64数据）
